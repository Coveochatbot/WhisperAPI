...
namespace WhisperAPI.Registries
{
    ...
    {
        public WhisperApiRegistry(string apiKey, List<string> irrelevantIntents, string nlpApiBaseAdress, string contextLifeSpan, string searchBaseAddress)
        {
            ...
            this.For<IIndexSearch>().Use<IndexSearch>().Ctor<string>("apiKey").Is(apiKey).Ctor<string>("searchBaseAddress").Is(searchBaseAddress);
            this.For<HttpClient>().AlwaysUnique().Use<HttpClient>();
